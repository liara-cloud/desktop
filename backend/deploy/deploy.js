...
} = require("@liara/cli/lib/utils/detect-platform.js");
const {
...
exports.deploy = async (event, args) => {
  try {
    ...
    event.sender.send(
      "deploy",
      generateLog("Deployment finished successfully.\n", "publish", "pending")
    );
    ...
    if (error instanceof BuildTimeout) {
      event.sender.send(
        "deploy",
        generateLog(
          "Build timed out. It tool about 10 minutes\n",
          "build",
          "error"
        )
      );
    ...
    if (error.message === "TIMEOUT") {
      event.sender.send(
        "deploy",
        generateLog(
          "Build timed out. It took about 10 minutes.",
          "build",
          "error"
        )
      );
